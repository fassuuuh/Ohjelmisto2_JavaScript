...
tvForm.addEventListener('submit', async function (evt) {
    ...
    try {
        ...
        for (const tvShow of jsonData) {
            ...
            // Add the medium image in an <img> element (with optional chaining)
            if (tvShow.show.image?.medium) {
                const imgElem = document.createElement('img');
                imgElem.src = tvShow.show.image.medium;
                imgElem.alt = tvShow.show.name;
                articleElem.appendChild(imgElem);
            }

            // Add the link to the details in an <a> element (place directly under the image)
